    ...
    if research_agent == "multi_agent":
        return {
            "supervisor_model": (
                ...
                or os.environ.get("SUPERVISOR_MODEL", "anthropic:claude-3-7-sonnet-latest")
            ),
            "researcher_model": (
                request.config.getoption("--researcher-model")
                or os.environ.get("RESEARCHER_MODEL", "anthropic:claude-3-5-sonnet-latest")
            ),
        }
    else:  # graph agent
        return {
...
def test_response_criteria_evaluation(research_agent, search_api, models, eval_model):
    ...
    console.print(Panel(Markdown(report), title="Generated Report", border_style="blue"))
    ...
    console.print(
        Panel.fit(
            f"[bold {result_color}]{result_text}[/bold {result_color}]", title="Evaluation Result"
        )
    )
    ...
    console.print(
        Panel(eval_result.justification, title="Evaluation Justification", border_style="yellow")
    )
